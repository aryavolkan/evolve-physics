...
use crate::{RigidBody, BodyHandle, CollisionEvent, Shape};
...
impl World {
    ...
    pub fn add_body(&mut self, body: &RigidBody) -> BodyHandle {
        ...
        let collider_handle = self.collider_set.insert_with_parent(collider, rb_handle, &mut self.rigid_body_set);
    ...
    pub fn remove_body(&mut self, handle: BodyHandle) -> Option<()> {
        ...
        let colliders: Vec<_> = self.rigid_body_set
            .get(rb_handle)?
            .colliders()
            .iter()
            .copied()
